...
if ($totPrecio == 0){
  ...
  function CreaArrayDescuentos($conexion,$clausulaNOTIN) {
			...
			$FormatCatalogo = "SELECT id_curso, preciotutorial, tiene_descuento, web_titulo  
		                         FROM vtcursos  
  ...
  function NumeroCursosYaComprados($conexion,$clausulaNOTIN) {
	   if (strlen($clausulaNOTIN) == 0){
	    ...
	    $totCursosYaComprados = 0;
			$FormatCursosYaComprados = "SELECT id_curso
		                                  FROM vtcursos  
		                                 WHERE vtcursos.esta_activo > 0 
		                                 and vtcursos.es_d_pago = 1 
...
if ($totCursosVenta == 0){
	...
	while ($rowCursosVenta = mysqli_fetch_assoc($resCursosVenta)) {
      $txtSaliente .="";
      $txtSaliente .= '<div class="cursosRelacionCol1"><input class="estilo-checkbox" type="checkbox" value="';
...
<script>
	function VerDescuentos(modo) {
		...
		k = document.getElementById('DivDescuentos');
...
function CambioValor(valor) {
		var cursosTotales = 0;   // Son los cursos de BBDD que tienen descuento, para calcular el porcentaje
...
function entregavalortotal(importe){

	let valor = importe;


alert("Escribe un correo a eduardo.mediavilla@cype.com indicando los cursos seleccionados y el valor de compra equivalente a:" + valor + "USD, para recibir instrucciones de pago y proceder con la inscripción. Estamos efectuando tareas de mantenimiento en nuestra pasarela de pagos. Si nos escribe un correo le facilitaremos un link de pago directo.");
...
<br>
	...
	<!--	<button id = "button123"  onclick="javascript:entregavalortotal(ImporteTotalCompra)">Valor de compra</button>   -->
       ...
       <br />
       <br />
      <div class ="centro"> <input id = "DEButton4" name="Salir" type="button" 
      	class ="ButtonGrisP" value ="&nbsp;&nbsp;&nbsp;&nbsp;Salir&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" 
      	onclick="javascript:VerDescuentos(0)"/></div>
       <br />
       <br /> 
