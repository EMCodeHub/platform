...
<SCRIPT LANGUAGE="JavaScript"> 
	   function procesar(sino) {
		  if (sino == 1){
              if (document.getElementById('etiqueta').value == "" || document.getElementById('pagina').value == "") {
                 alert("Informe los campos etiqueta y página"); 
                 return;  
             }
...
<title>Opciones de menú. Edición de Registros</title>
...
<?php
echo "<div class = TituloRegistro > Opciones de menú. Registro Núm ".$_REQUEST['registro'] ."</div>";
echo "<div class ='clear'></div>";
	include_once('../conexion/conn_bbdd.php');
	
	echo '<form id = "datos" name= "datos" action="VTOpcionesMenuLista.php" method="post">';
    ...
    $query = 'show columns FROM vtparamdatosmenu';
...
if ($_REQUEST['alta'] == 1) {
   ...
   $FormatMaestros = "select id, activo,llamada_directa_pagina, etiqueta, carpeta, pagina, orden from vtparamdatosmenu  where id = %d";
