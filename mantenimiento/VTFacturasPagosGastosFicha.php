...
if (!isset($_REQUEST['volver_a'])) {
	$_REQUEST['volver_a'] = "VTFacturasPagosGastosLista.php";
} else {

}
include_once('../php/VTFacturasPagosGastosScript.php');
...
//.....................................................
	   ...
	   function procesar(sino) {
		  if (sino == 1){
		  		 ...
		  		 if (trim(document.getElementById("proveedor").value).length < 5) {
	             	alert ("NO PROCESADO: Indique un proveedor");
					      document.getElementById("proveedor").focus();
	             	return false;
	             } 
					 
			
			 if (document.getElementById("importe").value <1) {
	             	alert ("NO PROCESADO:  Indique el importe ");
			 ...
			 if (Importe < Iva ) {
	             	alert ("NO PROCESADO:  Indique un iva menor ");
					      document.getElementById("iva").focus();
	             	return;
	             }
			    
			  
			  if  (trim(document.getElementById("medio_pago").value).length < 5) {
	             	alert ("NO PROCESADO: indique un medio de pago: Banco, Tarjeta ...");
					      document.getElementById("medio_pago").focus();
	             	return;
	             }
...
echo "<div class = 'TituloCurso' > Facturas Pagadas, Gastos Registro Núm ".$_REQUEST['registro'];
echo "</div>";
echo "<br>";

	
	echo '<form id = "datos" name= "datos" action="VTFacturasPagosGastosLista.php" method="post">';
    ...
    $query = 'show columns FROM vtpagosgastos';
...
if ($_REQUEST['alta'] == 1) {
	      ...
	      //.....................
			...
			if ($CampoMombre[$i] == '**id_receta') {
				echo elinput($CampoMombre[$i],$_REQUEST['id_solicitud'], $CampoTipo[$CampoMombre[$i]],$conexion);  
			} else {
