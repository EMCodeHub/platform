...
$queMaestros = sprintf($FormatMaestros, $_POST['Dia'],date("Y-m-d"),$_POST["Correo"] , $_POST["Hora"], $_POST["Hora"] +1,$_POST["Ciudad"], $_POST["UsuSkype"],$_POST["Telefono"],$_POST["Pais"],str_replace("<","[",$_POST["Observaciones"]),$_POST["Nombre"],$_POST["Apellidos"]);
...
if ($solicitudRepetida == 0) {
	$FormatMaestros = "INSERT INTO compracypealumnos (activo, f_alta, f_confirmacion, f_sesion, h_inicio, h_final, ciudad, 
                             skype_usuario, telefono, email, pais, observa_cliente, nombre, apellidos  )
                      values (%d,'%s', null , '%s', %d, %d, '%s', '%s', '%s', '%s', '%s', '%s',  '%s', '%s') ";
    $queMaestros = sprintf($FormatMaestros, 1,date("Y-m-d"), $_POST['Dia'],$_POST['Hora'] ,$_POST['Hora'] + 1, $_POST['Ciudad'], $_POST['UsuSkype'], $_POST['Telefono'], $_POST['Correo'],  $_POST['Pais'],  str_replace("<","[",$_POST["Observaciones"]),  $_POST['Nombre'], $_POST['Apellidos']);
