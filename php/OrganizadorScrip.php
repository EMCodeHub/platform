...
///////////////////MaestrosLista///////////////////////////////////////////////////
function pintaOrganizadores($conexion) {
    //.................cabecera del fichero
    $query = 'show columns FROM organizadores';
...
if ($_REQUEST['operacion'] == "UPDATE"){
    $FormatMaestros = "UPDATE  organizadores SET ";
    $n = 0;
	$ValorGrabar = "";
...
if ($_REQUEST['operacion'] == "INSERT"){
	    $FormatMaestros = "INSERT INTO organizadores (";
    $n = 0;
    for( $i = 0; $i < $nColumnas ; $i++ ) {
	     if($n == 0){
		    $coma = "";
...
$FormatMaestros = "select id,	descripcion_interna,	nombre_organizador,	pais,	ciudad,	lugar, direccion,	pers_contacto, tfn_contacto from organizadores";
...
if ($totMaestros> 0) {  //.....Registro de conexión
    echo "<div class = rowCabeceraOrganizador >";
   for( $i = 0; $i < $nColumnas ; $i++ ) {
   ...
   while ($rowRegistros = mysqli_fetch_assoc($resMaestros)) {
	          ...
	          echo "<div class = rowElementoOrganizador onclick='edita(".$rowRegistros[$CampoMombre[0]].")'>";  //siempre pasamos el primer campo, es la id QuimGrupos
			  for( $i = 0; $i < $nColumnas ; $i++ ) {
