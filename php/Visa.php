...
$FormatMaestros = "SELECT id as ID_PROD, id_curso
...
if ($totMaestros > 0){
	 while ($rowVTMaestros = mysqli_fetch_assoc($resMaestros)) {
		 ...
		 $curso_id = $rowVTMaestros['id_curso'];
	 }	
} else {
 ...
 /////////////////////////////////////cursos//////////////////////////
 $FormatMaestros = "SELECT titulo_cur, descripcion_cur, preciotutorial
                   FROM vtcursos
                   WHERE  id_curso =  '%d'
				 ";
$queMaestros = sprintf($FormatMaestros,$curso_id);
...
if ($totMaestros > 0){
	 while ($rowVTMaestros = mysqli_fetch_assoc($resMaestros)) {
		 $name = $rowVTMaestros['titulo_cur'];
		 $description = $rowVTMaestros['descripcion_cur'];
		 $price = $rowVTMaestros['preciotutorial'];
		  if (strlen ( $name ) > 120) {
...
<title><?php echo $DatosWeb->GetTrimValor('web_dominio') ?>  Compra de Videotutorial</title>
</head>
