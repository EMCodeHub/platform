...
function esTextArea($nombre) {
	...
	if ($nombre == 'titulo_bloc'){
		return TRUE;
	}
	if ($nombre == 'descripcion_bloc'){
		return TRUE;
	}
	return false;
}
...
*/				 
	...
	if ($pos !== false) {
	  ...
	  if ($nombre == 'id_vtcurmodulo'){  //es un campo indice hay que hacer una select a su fichero
	      $return = hazSelectIndiceModulo("vtcursomodulo","id_modulo",$valor,$conexion,"id_vtcurmodulo",0,1); 
		  return $return; //..................................................................................input propio
	  }
...
///////////////////MaestrosLista///////////////////////////////////////////////////
function pintaVTBloques($conexion) {
...
if ($_REQUEST['operacion'] == "UPDATE"){
    $FormatMaestros = "UPDATE  vtcurmodbloque SET ";
    $n = 0;
	$ValorGrabar = "";
    ...
    $FormatMaestros = $FormatMaestros." WHERE  id_bloque = ".$_REQUEST['registro'];
...
if ($_REQUEST['operacion'] == "INSERT"){
	    $FormatMaestros = "INSERT INTO vtcurmodbloque (";
    $n = 0;
    for( $i = 0; $i < $nColumnas ; $i++ ) {
	     if($n == 0){
		    $coma = "";
...
$FormatMaestros = "SELECT  id_bloque, id_vtcurmodulo, vtcurmodbloque.esta_activo, orden_bloc, titulo_bloc, descripcion_bloc,imaicono_bloc  
                   FROM    vtcursomodulo, vtcursos, vtcurmodbloque
				   where  vtcursomodulo.id_vtcurso = vtcursos.id_curso
				     and   vtcurmodbloque.id_vtcurmodulo = vtcursomodulo.id_modulo
				   ORDER   BY orden, orden_mod, orden_bloc";
    ...
    if ($_REQUEST['filtro'] == "FILTRO_ID" && $_REQUEST['id_vtcurmodulo'] !=0) {
		
	    $FormatMaestros = "SELECT  id_bloque, id_vtcurmodulo, vtcurmodbloque.esta_activo, orden_bloc, titulo_bloc, descripcion_bloc,imaicono_bloc   
		                   FROM vtcurmodbloque  
						   where id_vtcurmodulo = %d 
						   ORDER BY orden_bloc";

	    $queMaestros = sprintf($FormatMaestros,$_REQUEST['id_vtcurmodulo']); 
...
if ($totMaestros> 0) {  //.....Registro de conexión
   ...
   while ($rowRegistros = mysqli_fetch_assoc($resMaestros)) {
	          ...
	          echo '<A NAME="'.$rowRegistros['id_bloque'].'">';
...
///////////////////MaestrosLista///////////////////////////////////////////////////
function hazSelectIndiceModulo($fichero,$campo,$valorID,$conexion,$nombreSelect,$activarBlanco,$open_close) {
 ...
 $txtSelect ="";
  //$FormatImagen = "select %s, titulo_mod  from %s  order by orden_mod ";
    $FormatImagen = "select %s, id_vtcurso, titulo_mod from %s   order by id_vtcurso, %s ";

  
  $queImagen = sprintf($FormatImagen, $campo, $fichero,$campo); 
...
//echo "@hoyhoy@...entro en el ciclo..."; //debug
	   ...
	   if ($rowImagen[$campo] == $valorID) {
		 ...
		 $valorOpcion = $rowImagen['id_vtcurso']."-".$rowImagen[$campo]."-".$rowImagen['titulo_mod'];	  
...
/* if ($totImagen > 1) {  // Eliminamos la seccion General
 ...
 if ($totImagen > 0) {  //.....Registro de conexión
     $txtSelect = $txtSelect.'<select '.$Activado.' id = '.$nombreSelect.' name="'.$nombreSelect.'"';                  
	  ...
	  $txtSelect = $txtSelect.' onchange="SeleccionaModulo(this)"';
...
////////////////////////////////////////////////////////////////////////////
function Descripcion1Curso($idModulo,$conexion) {
   $devolver ="";
   $FormatVTCursos = "SELECT titulo_cur 
                      FROM vtcursos, vtcursomodulo 
					  WHERE  vtcursos.id_curso = vtcursomodulo.id_vtcurso
					         and  id_modulo = %d";
   $queVTCursos = sprintf($FormatVTCursos, $idModulo); 
...
////////////////////////////////////////////////////////////////////////////
function Descripcion1Modulo($idModulo,$conexion) {
   $devolver ="";
