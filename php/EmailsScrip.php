...
function elinput($nombre, $valor,$tipo,$conexion) {
	...
	if ($pos !== false) {
	  ...
	  if ($nombre == 'tipocorreo'){  //es un campo indice hay que hacer una select a su fichero
	      $return = hazSelectIndice("emailstipos","id",$valor,$conexion,"tipocorreo",0); 
		  return $return; //..................................................................................input propio
	  }
...
///////////////////MaestrosLista///////////////////////////////////////////////////
function pintaEmails($conexion) {
    $query = 'show columns FROM emailscomerciales';
...
if ($_REQUEST['operacion'] == "UPDATE"){
    $FormatMaestros = "UPDATE  emailscomerciales SET ";
    $n = 0;
	$ValorGrabar = "";
...
if ($_REQUEST['operacion'] == "INSERT"){
	    $FormatMaestros = "INSERT INTO emailscomerciales (";
    $n = 0;
    for( $i = 0; $i < $nColumnas ; $i++ ) {
	     if($n == 0){
		    $coma = "";
...
$FormatMaestros = "select id, correoelectronico, nombre_interno, nombre_correo, es_smtp, es_pop3, servidor, 
puerto, seguridad, requiere_auth, usuario, password, usa_logo, fichero_logo, tipocorreo   from emailscomerciales order by tipocorreo";
...
if ($totMaestros> 0) {  //.....Registro de conexión
    echo "<div class = rowCabeceraEmail >";
   for( $i = 0; $i < $nColumnas ; $i++ ) {
   ...
   while ($rowRegistros = mysqli_fetch_assoc($resMaestros)) {
	          ...
	          echo "<div class = rowElementoEmail onclick='edita(".$rowRegistros[$CampoMombre[0]].")'>";  //siempre pasamos el primer campo, es la id QuimGrupos
			  for( $i = 0; $i < $nColumnas ; $i++ ) {
...
$pos = strpos($mombreCampo,'fichero_logo');	
	if ($pos !== false) {
		return '<div class="IzquierdaGrupo"><img src="../imagenes/logos/'.$contenidoCampo.'" width="40" height="40" /></div>'."<div class='DerechaGrupo'>".$contenidoCampo."</div>";
	} 
...
function hazSelectIndice($fichero,$campo,$valorID,$conexion,$nombreSelect,$activarBlanco) {
 ...
 $txtSelect ="";
  $FormatImagen = "select %s, descripcion from %s  order by id ";
...
//echo "@@...entro en el ciclo..."; //debug
	   ...
	   if ($rowImagen[$campo] == $valorID) {
		 ...
		 $txtSelect = $txtSelect. '<option value = "'.$rowImagen[$campo].'" '.$selected.'>'.$rowImagen['id']."-".$rowImagen['descripcion'];
...
/////////////////////////////////////////////////////////////////////
function CuentaTipoCorreo($tipo,$conexion) {
  $numero = 0;    
  $FormatMaestros = "SELECT count(id) AS contador FROM emailscomerciales where tipocorreo = %d";
  $queMaestros = sprintf($FormatMaestros, $tipo); 
  $resMaestros = mysqli_query($conexion, $queMaestros) or die(mysqli_error($conexion));
  $totMaestros = mysqli_num_rows($resMaestros);
   while ($rowRegistros = mysqli_fetch_assoc($resMaestros)) {
		$numero = $rowRegistros['contador'];
	}      
