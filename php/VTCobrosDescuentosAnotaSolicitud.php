...
$FormatMaestros = "SELECT id
                   ...
                   WHERE  RTRIM(LOWER(email_cliente)) =   RTRIM(LOWER('%s'))
					 ...
					 and tipomensaje   =  '%s'
				 ";
$queMaestros = sprintf($FormatMaestros,$_POST['DEemail'], $_POST['loteCursos'], date("Y-m-d"), $_POST['tipomensaje']);
...
$queMaestros = sprintf($FormatMaestros, $_POST['DENumIdCurso'],$_POST['loteCursos'], $_POST['DEimporte'],date("Y-m-d"), $_POST['tipomensaje'], $_POST['DEemail'], $_POST['mailemisor'],$_POST['agente']);
...
if ($totMaestros > 0){
 ...
 if ($mailRepetido == 0 &&  $escribirAOrganizador > 0) {
	$titulocorreo  = $_REQUEST["tipomensaje"].':  '.$_REQUEST["DEemail"]."\n\n";
    $cuerpoComercial = "=== SOLICITANTE  ===================================================\n";
    $cuerpoComercial .= "EMAIL: ".$_REQUEST["DEemail"]."\n";
    $cuerpoComercial .= "=== SOLICITUD DE COMPRA LOTE DE CURSOS  CON DESCUENTO ==============\n";
    $cuerpoComercial .= "NUMEROS DE CURSO: ".$_REQUEST["loteCursos"]."\n";
    $cuerpoComercial .= "===================================================================\n\n";
    $cuerpoComercial .= "El cliente intenta comprar con descuentos. Mira la tabla de solicitudes\n";
