...
function elinput($nombre, $valor,$tipo,$conexion) {
	...
	if ($pos !== false) {
		...
		if ($nombre == 'landingpage'){
		    $carpeta = "../paginas/landingpages";	
		}
   ...
   //.....................................
	...
	if ($long <= 70) {
		...
		$colu = ($long /70 ) +1;
		return '<textarea name="'.$nombre.'"  id="'.$nombre.'"   cols="70" rows="'.$colu.'">'.$valor.'</textarea>';

	}
}
///////////////////MaestrosLista///////////////////////////////////////////////////
function pintaVTTipoAlumno($conexion) {
...
if ($_REQUEST['operacion'] == "UPDATE"){
    $FormatMaestros = "UPDATE  vttipoalumno SET ";
    $n = 0;
	$ValorGrabar = "";
...
if ($_REQUEST['operacion'] == "INSERT"){
	    $FormatMaestros = "INSERT INTO vttipoalumno (";
    $n = 0;
    for( $i = 0; $i < $nColumnas ; $i++ ) {
	     if($n == 0){
		    $coma = "";
...
if ($totMaestros> 0) {  //.....Registro de conexión
   ...
   while ($rowRegistros = mysqli_fetch_assoc($resMaestros)) {
	          ...
	          echo "<div class = rowElementoTipoAlumno onclick='edita(".$rowRegistros[$CampoMombre[0]].")'>";  //siempre pasamos el primer campo, es la id QuimGrupos
			  for( $i = 0; $i < $nColumnas ; $i++ ) {
