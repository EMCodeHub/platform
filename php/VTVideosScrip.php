...
function esTextArea($nombre) {
	...
	if ($nombre == 'titulo_video'){
		return TRUE;
	}
		if ($nombre == 'url_YouTube'){
		return TRUE;
	}
...
///////////////////MaestrosLista///////////////////////////////////////////////////
function pintaVTVideos($conexion) {
...
if ($_REQUEST['operacion'] == "UPDATE"){
    $FormatMaestros = "UPDATE  vtcurmodbloqvideo SET ";
    $n = 0;
	$ValorGrabar = "";
...
if ($_REQUEST['operacion'] == "INSERT"){
	    $FormatMaestros = "INSERT INTO vtcurmodbloqvideo (";
    $n = 0;
    for( $i = 0; $i < $nColumnas ; $i++ ) {
	     if($n == 0){
		    $coma = "";
...
$FormatMaestros = "SELECT  id, vtcurmodbloqvideo.esta_activo, id_vtcurmodbloque, es_de_pago, orden_video, titulo_video, url_YouTube, video_pago 
                   FROM    vtcursomodulo, vtcursos, vtcurmodbloque, vtcurmodbloqvideo
				   ...
				   ORDER   BY orden, orden_mod, orden_bloc, orden_video";
...
function Descripcion3Modulo($idRegistro,$conexion) {
   ...
   $FormatVTCursos = "SELECT titulo_mod 
                      FROM vtcursomodulo, vtcurmodbloque, vtcurmodbloqvideo
...
function Descripcion3Bloque($idRegistro,$conexion) {
   ...
   $FormatVTCursos = "SELECT titulo_bloc 
                       FROM vtcurmodbloque, vtcurmodbloqvideo
