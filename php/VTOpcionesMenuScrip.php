...
///////////////////MaestrosLista///////////////////////////////////////////////////
function pintaVTOpcionesMenu($conexion) {
...
if ($_REQUEST['operacion'] == "UPDATE"){
    $FormatMaestros = "UPDATE  vtparamdatosmenu SET ";
    $n = 0;
	$ValorGrabar = "";
...
if ($_REQUEST['operacion'] == "INSERT"){
	    $FormatMaestros = "INSERT INTO vtparamdatosmenu (";
    $n = 0;
    for( $i = 0; $i < $nColumnas ; $i++ ) {
	     if($n == 0){
		    $coma = "";
...
$FormatMaestros = "select id, activo,llamada_directa_pagina, etiqueta, carpeta, pagina, orden from vtparamdatosmenu order by orden";
...
if ($totMaestros> 0) {  //.....Registro de conexión
    echo "<div class = 'rowOpcionesMenu' >";
   for( $i = 0; $i < $nColumnas ; $i++ ) {
   ...
   while ($rowRegistros = mysqli_fetch_assoc($resMaestros)) {
	          ...
	          echo "<div class = 'rowOpcionesMenu' onclick='edita(".$rowRegistros[$CampoMombre[0]].")'>";  //siempre pasamos el primer campo, es la id QuimGrupos
			  for( $i = 0; $i < $nColumnas ; $i++ ) {
