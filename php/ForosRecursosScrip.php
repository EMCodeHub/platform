...
function elinput($nombre, $valor,$tipo,$conexion) {
	...
	if ($pos !== false) {
		...
		//}
		if ($nombre == 'ficherorecurso'){
		   $carpeta = "../Foros/Recursos";	
		}
	...
	if ($pos !== false) {
	  ...
	  $valida = " onKeypress='return solonumeros()'";
	  	if ($nombre == 'id_forosmensajes'){  //es un campo indice hay que hacer una select a su fichero
	      $return = hazSelectIndiceForosMensajes("forosmensajes","id",$valor,$conexion,"id_forosmensajes",0,1); 
		  return $return; //..................................................................................input propio
	    }
...
///////////////////MaestrosLista///////////////////////////////////////////////////
function pintaRecursos($conexion) {
    $query = 'show columns FROM forosrecursos';
...
if ($_REQUEST['operacion'] == "UPDATE"){
    $FormatMaestros = "UPDATE  forosrecursos SET ";
    $n = 0;
	$ValorGrabar = "";
	...
	ActualizaNumRecursos($_REQUEST['registro_mensaje'],$conexion);
} 
if ($_REQUEST['operacion'] == "INSERT"){
	    $FormatMaestros = "INSERT INTO forosrecursos (";
    $n = 0;
    for( $i = 0; $i < $nColumnas ; $i++ ) {
	     if($n == 0){
		    $coma = "";
    ...
    if ($_REQUEST['sel_pendientes'] == 1 ) {
        $operador = " != ";
        if ($_REQUEST['registro_mensaje'] !=0) {
            $operador = " = ";
        }
    
             $FormatMaestros = "SELECT id, id_forosmensajes, id_alumno, titulo, ficherorecurso, fecha_alta, esta_activo, fecha_baja, fecha_revision
		                FROM forosrecursos
				       WHERE esta_activo >0 
                         and id_forosmensajes ".$operador." %d 
				         and (fecha_revision <'1999-01-01' OR fecha_revision is NULL) 
                       ORDER by fecha_alta";
    
             $queMaestros = sprintf($FormatMaestros,$_REQUEST['registro_mensaje'] ); 
    
        
        
    
    } elseif ($_REQUEST['filtro'] == "FILTRO_ID" && $_REQUEST['registro_mensaje'] !=0 && $_REQUEST['FechaFiltro'] <> "" && $_REQUEST['FechaFiltro'] <> "0000-00-00"){
   ...
   $FormatMaestros = "SELECT id, id_forosmensajes, id_alumno, titulo, ficherorecurso, fecha_alta, esta_activo, fecha_baja, fecha_revision
		                FROM forosrecursos
				       WHERE forosrecursos.id_forosmensajes = %d 
				         and fecha_alta >= '%s' 
                       ORDER by id";
		                   
    $queMaestros = sprintf($FormatMaestros,$_REQUEST['registro_mensaje'],$_REQUEST['FechaFiltro'] ); 
	
	 //echo "hoyhoy".$queMaestros;
	
	} elseif ($_REQUEST['filtro'] == "FILTRO_ID" && $_REQUEST['registro_mensaje'] != 0 ){
        	//echo "estamos en 2";
   $FormatMaestros = "SELECT id, id_forosmensajes, id_alumno, titulo, ficherorecurso, fecha_alta, esta_activo, fecha_baja, fecha_revision
		                FROM forosrecursos
				       WHERE forosrecursos.id_forosmensajes = %d 
...
function hazSelectIndiceFiltro($fichero,$campo,$valorID,$conexion,$nombreSelect,$activarBlanco,$open_close) {
  ...
  $txtSelect ="";
  $FormatImagen = "select %s, mensaje  from %s  order by mensaje ";
...
//echo "...filas....".$totImagen; //debug
  ...
  if ($totImagen> 0) {  //.....Registro de conexión
	...
	$txtSelect = $txtSelect.' onchange="SeleccionaMensaje(this)"';
	 
	 $txtSelect = $txtSelect.'>';
	 if ($activarBlanco == 1) {
		 $txtSelect = $txtSelect. '<option value = "0" >Todos los Mensajes';
	 }
...
//echo "@@...fuera del ciclo..."; //debug
     ...
     while ($rowImagen = mysqli_fetch_assoc($resImagen)) {
	   ...
	   if ($rowImagen[$campo] == $valorID) {
		 ...
		 $valorEtiqueta = substr($rowImagen['id']."-".$rowImagen['mensaje'],0,70);	    
...
///////////////////MaestrosLista///////////////////////////////////////////////////
function hazSelectIndiceForosMensajes($fichero,$campo,$valorID,$conexion,$nombreSelect,$activarBlanco,$open_close) {
...
//echo "@hoyhoy@...entro en el ciclo..."; //debug
	   ...
	   if ($rowImagen[$campo] == $valorID) {
		 ...
		 $txtSelect = $txtSelect. '<option value = "'.$rowImagen[$campo].'" '.$selected.'>'.$rowImagen['mensaje'];
...
//..............................................................
function ActualizaNumRecursos($mensaje,$conexion) {
$numero= 0;
...
$queTemas = sprintf($FormatTemas,$mensaje);
...
$queTemas = sprintf($FormatTemas,$numero,$mensaje);
$resTemas = mysqli_query($conexion, $queTemas) or die(mysqli_error($conexion)); 
mysqli_free_result($resTemas);  
}
//.............................................................
function Volver(){
    if ($_REQUEST['registro_mensaje'] != 0){
       return $_REQUEST['volver_recurso'];
    } else {
